...
`
		Expect(RenderXHTML(source, configuration.WithFilename("tmp/foo.adoc"))).To(Equal(expectedContent))
	})
...
`
		Expect(RenderXHTML(source, configuration.WithAttributes(attrs))).To(Equal(expected))
	})
